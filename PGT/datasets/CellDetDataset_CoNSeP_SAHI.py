...
class CellDetDataset_CoNSeP_SAHI(CustomDataset):
    CLASSES = ('Inflammatory', 'Epithelial', 'Spindle-shaped')
    ...
    def det(self, pred_centroid, pred_inst_type, img_name, img_idx):
        img_path = os.path.join(self.data_root, "Labels", img_name + ".mat")
    ...
    def evaluate(self,
        ...
        for idx, result in enumerate(results):
            ...
            img_name = ret.group(1)
            h_offset = int(ret.group(2))
            w_offset = int(ret.group(3))

            if h_offset % 250 != 0 or w_offset % 250 != 0:
                continue
            if idx == 0:
                pred_centroid = []
                pred_inst_type = []
            ...
            elif (h_offset == 0 and w_offset == 0):
                self.det(pred_centroid, pred_inst_type, img_name_last, img_idx)
                img_name_last = img_name
                # pred info
                pred_centroid = []
                pred_inst_type = []
            ...
            if idx == len(results) - 1:
                self.det(pred_centroid, pred_inst_type, img_name, img_idx)
        ...
        for i in range(len(self.cat_ids)):
            ...
            mf1c += results_list[i + 2]
        print_log("===========================Evaluation=======================\n", logger=logger)
        results_dict['mF1c'] = mf1c / len(self.CLASSES)
        return results_dict
