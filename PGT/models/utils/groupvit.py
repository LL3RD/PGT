...
class GroupingBlockWithoutMLP(nn.Module):
    ...
    def __init__(self,
                 *,
                 ...
                 assign_eps=1.,
                 gumbel_tau=1.):
        super(GroupingBlockWithoutMLP, self).__init__()
        self.dim = dim
        self.hard = hard
        self.gumbel = gumbel
        self.sum_assign = sum_assign
...
class Group_Classifier_Attn(nn.Module):
    def __init__(self,
                 embed_dim=256,
                 num_heads=[8, 8],
                 num_group_tokens=[64, 3],
                 num_output_groups=[64, 3],
                 hard_assignment=True):
        super().__init__()
...
class Group_Classifier(nn.Module):
    def __init__(self,
                 embed_dim=256,
                 num_heads=[8, 8],
                 num_group_tokens=[64, 3],
        ...
        group_token = []
        group_class_head = []
        for i in range(len(num_group_tokens)):
            group_token.append(nn.Parameter(torch.zeros(1, num_group_tokens[i], embed_dim)))
            group_head = GroupingBlock(
                dim=embed_dim,
                out_dim=embed_dim,
                num_heads=num_heads[i],
                num_group_token=num_group_tokens[i],
                num_output_group=num_output_groups[i],
                norm_layer=norm_layer,
                hard=hard_assignment,
                gumbel=hard_assignment,
            )
            group_class_head.append(group_head)
        self.group_token = nn.ParameterList(group_token)
        self.group_class_head = nn.ModuleList(group_class_head)

    def forward(self, x, return_attn=False):
        L, B, C = x.shape
        x = x.transpose(0, 1)
        query = self.shortcut(x)
        attn_dict_list = []
        for i, cls_head in enumerate(self.group_class_head):
            group_token = self.group_token[i].expand(B, -1, -1)
            x, attn_dict = cls_head(x, group_token, return_attn=return_attn)
            if return_attn:
                attn_dict_list.append(rearrange(query @ x.transpose(1, 2), 'b n c -> n b c'))
        cls = rearrange(query @ x.transpose(1, 2), 'b n c -> n b c')
        if return_attn:
            return cls, attn_dict_list
        else:
            return cls


class Group_Classifier_Prompt(nn.Module):
    def __init__(self,
                 embed_dim=256,
                 num_heads=[8, 8],
                 num_group_tokens=[64, 3],
        ...
        # group_token = []
        group_class_head = []
        self.group_token_last = nn.Parameter(torch.zeros(1, num_group_tokens[-1], embed_dim))
        for i in range(len(num_group_tokens)):
            # group_token.append(nn.Parameter(torch.zeros(1, num_group_tokens[i], embed_dim)))
            group_head = GroupingBlockWithoutMLP(
                dim=embed_dim,
                out_dim=embed_dim,
                num_heads=num_heads[i],
                num_group_token=num_group_tokens[i],
    ...
    def forward(self, x, return_attn=False, gp_for_prompt=None):
        L, B, C = x.shape
        x = x.transpose(0, 1)
        query = self.shortcut(x)
        attn_dict_list = []
        for i, cls_head in enumerate(self.group_class_head):
            if i == 0:
                group_token = self.gp_mlp(gp_for_prompt.expand(B, -1, -1))
            else:
                group_token = self.group_token_last.expand(B, -1, -1)
            x, attn_dict = cls_head(x, group_token, return_attn=return_attn)
            if return_attn:
                attn_dict_list.append(rearrange(query @ x.transpose(1, 2), 'b n c -> n b c'))
        cls = rearrange(query @ x.transpose(1, 2), 'b n c -> n b c')
...
class Group_Classifier_WithoutReduction(nn.Module):
    def __init__(self,
                 embed_dim=256,
                 num_heads=[8, 8],
                 num_group_tokens=[64, 3],
...
if __name__ == '__main__':
    ...
    # GroupViT_Model = GroupViT(num_classes=3)
    # output = GroupViT_Model(input)
    input = torch.zeros((1000, 2, 256))
    group_cls = Group_Classifier_WithoutReduction(256)
    output, attn_dict_list = group_cls(input, True)
    print(output.shape)
    print(attn_dict_list[0]['soft'].shape)
    print(attn_dict_list[1]['soft'].shape)
    # print(1)
